...
export default function PropostasPage() {
  ...
  function getStatusBadge(status) {
    const statusConfig = {
      ...
      aguardando_cliente: { label: "Aguardando Cliente", color: "bg-gray-50 text-gray-700 border border-gray-200" },
    }
                ...
                return (
                  <tr key={`${proposta.origem}-${proposta.id}`} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4">
                      ...
                      <div className="text-xs text-gray-500" title={proposta.corretor_nome || "N/A"}>
                        {proposta.corretor_nome || "N/A"}
                      </div>
                    </td>
                    <td className="px-4 py-4">
              ...
              {loadingDetalhes ? (
                ...
                <div className="space-y-8">
                  ...
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    ...
                    {dependentes.length > 0 ? (
                      <div className="space-y-6">
                        {dependentes.map((dependente, index) => (
                          <div key={index} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                            <h4 className="font-semibold text-gray-900 mb-3">Dependente {index + 1}</h4>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              <div>
                              ...
                              <div>
                                ...
                                <p className="text-gray-900">
                                  {getParentescoAmigavel(dependente.parentesco) || "Não informado"}
                                </p>
                              </div>
                              <div>
                                <label className="block text-sm font-medium text-gray-600">Data de Nascimento</label>
                                <p className="text-gray-900">
                                  {dependente.data_nascimento
                                    ? `${formatarDataSegura(dependente.data_nascimento)} (${calcularIdade(dependente.data_nascimento)})`
                                    : "Não informado"}
                                </p>
                              </div>
                              <div>
                              ...
                              {dependente.valor_individual && (
                                <div>
                                  <label className="block text-sm font-medium text-gray-600">Valor Individual</label>
                                  <p className="text-gray-900 font-medium">
                                    R${" "}
                                    {Number(dependente.valor_individual).toLocaleString("pt-BR", {
                                      minimumFractionDigits: 2,
                                    })}
                                  </p>
                                </div>
                            ...
                            {(() => {
                              const docsDependente = obterDocumentosInteligente(dependente, "dependente")
                              return (
                                Object.keys(docsDependente).length > 0 && (
                                  <div className="mt-4">
                                    <label className="block text-sm font-medium text-gray-600 mb-2">Documentos</label>
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                      {Object.entries(docsDependente).map(([tipo, url]) => (
                                        <a
                                          key={tipo}
                                          href={url}
                                          target="_blank"
                                          rel="noopener noreferrer"
                                          className="text-blue-600 hover:text-blue-800 text-sm bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded border border-blue-200 transition-colors"
                                        >
                                          {getNomeDocumento(tipo)}
                                        </a>
                                      ))}
                                    </div>
                                  </div>
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <p>Nenhum dependente cadastrado nesta proposta</p>
                      </div>
                    )}
                  </div>

                  {/* 6. Documentos do Titular */}
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    <h3 className="text-xl font-semibold mb-4 text-gray-900 border-b border-gray-200 pb-2">
                      Documentos do Titular
                    </h3>
                    {(() => {
                      const documentosTitular = obterDocumentosInteligente(propostaDetalhada, "titular")
                      return Object.keys(documentosTitular).length > 0 ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          {Object.entries(documentosTitular).map(([tipo, url]) => (
                            <div
                              key={tipo}
                            ...
                            >
                              ...
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-blue-600 hover:text-blue-800 text-sm bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded border border-blue-200 transition-colors inline-block w-full text-center"
                              >
                                Visualizar {getTipoArquivo(url) === "imagem" ? "Imagem" : "Documento"}
                              </a>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="text-center py-8 text-gray-500">
                          <p>Nenhum documento foi enviado pelo titular</p>
                        </div>
                      )
                    })()}
                  </div>

                  {/* 7. Dados de Saúde do Titular */}
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    <h3 className="text-xl font-semibold mb-4 text-gray-900 border-b border-gray-200 pb-2">
                      Questionário de Saúde - Titular
                    </h3>
                    {questionario.length > 0 ? (
                      <div className="space-y-3">
                        {questionario.map((resposta, index) => (
                          <div key={index} className="border-l-4 border-blue-200 pl-4 py-2">
                            <p className="text-sm font-medium text-gray-700">
                              Pergunta {resposta.pergunta_id}: {resposta.pergunta_texto || "Pergunta não disponível"}
                            </p>
                            <p
                              className={`text-sm mt-1 ${resposta.resposta === "sim" ? "text-red-600 font-medium" : "text-green-600"}`}
                            >
                              Resposta: {resposta.resposta === "sim" ? "SIM" : "NÃO"}
                            </p>
                            {resposta.detalhes && (
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <p>Questionário de saúde não foi preenchido pelo titular</p>
                      </div>
                    )}
                  </div>

                  {/* 8. Dados de Saúde dos Dependentes */}
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    <h3 className="text-xl font-semibold mb-4 text-gray-900 border-b border-gray-200 pb-2">
                      Questionário de Saúde - Dependentes
                    </h3>
                    {questionarioDependentes.length > 0 ? (
                      <div className="space-y-6">
                        {questionarioDependentes.map((depQuest, depIndex) => (
                          <div key={depIndex} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                            <h4 className="font-semibold text-gray-900 mb-3">
                              {depQuest.dependente_nome} ({getParentescoAmigavel(depQuest.dependente_parentesco)})
                            </h4>
                            <div className="space-y-3">
                              {depQuest.questionario.map((resposta, index) => (
                                <div key={index} className="border-l-4 border-purple-200 pl-4 py-2">
                                  <p className="text-sm font-medium text-gray-700">
                                    Pergunta {resposta.pergunta_id}:{" "}
                                    {resposta.pergunta_texto || "Pergunta não disponível"}
                                  </p>
                                  <p
                                    className={`text-sm mt-1 ${resposta.resposta === "sim" ? "text-red-600 font-medium" : "text-green-600"}`}
                                  >
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <p>Nenhum questionário de saúde preenchido pelos dependentes</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    ...
                    {propostaDetalhada.assinatura_digital ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="block text-sm font-medium text-gray-600">Data da Assinatura</label>
                          <p className="text-gray-900">{formatarDataSegura(propostaDetalhada.data_assinatura)}</p>
                        </div>
                        <div>
                          <label className="block text-sm font-medium text-gray-600">IP do Cliente</label>
                          <p className="text-gray-900">{propostaDetalhada.ip_assinatura || "Não registrado"}</p>
                        </div>
                        <div className="md:col-span-2">
                          <label className="block text-sm font-medium text-gray-600 mb-2">Assinatura</label>
                          <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                            <img
                              src={propostaDetalhada.assinatura_digital || "/placeholder.svg"}
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <p>Proposta ainda não foi assinada digitalmente</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    ...
                    {propostaDetalhada.pdf_url ? (
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-gray-700">PDF da proposta gerado automaticamente</p>
                          <p className="text-sm text-gray-500">Clique para visualizar ou baixar</p>
                        </div>
                        <div className="flex gap-3">
                          <a
                            href={propostaDetalhada.pdf_url}
                            target="_blank"
                            rel="noopener noreferrer"
                      ...
                      <div className="text-center py-8 text-gray-500">
                        <p>PDF da proposta ainda não foi gerado</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-white p-6 rounded-lg border border-gray-200">
                    ...
                    {propostaDetalhada.observacoes ? (
                      <div className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <p className="text-gray-900 whitespace-pre-wrap">{propostaDetalhada.observacoes}</p>
                      </div>
                    ) : (
                      <div className="text-center py-8 text-gray-500">
                        <p>Nenhuma observação foi adicionada a esta proposta</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-gray-50 p-6 rounded-lg border border-gray-300">
                    <h3 className="text-lg font-semibold mb-3 text-gray-800">
                      Dados Técnicos - Debug ({propostaDetalhada.tabela_origem})
                    </h3>
                    <div className="text-xs text-gray-600 space-y-2 bg-white p-3 rounded border max-h-60 overflow-y-auto">
                      <pre className="whitespace-pre-wrap">{JSON.stringify(propostaDetalhada, null, 2)}</pre>
                    </div>
                  </div>
                </div>
              )}
