 ...
 */
export async function salvarDependentes(propostaId: string, dependentes: any[]): Promise<any> {
  try {
    ...
    const dependentesData = dependentes.map((dep, index) => {
      ...
      return {
        ...
        uf_nascimento: dep.uf_nascimento || "SP",
    ...
    if (error) {
      ...
      if (error.message && error.message.includes("Invalid API key")) {
        ...
        if (reconectado) {
          ...
          return await salvarDependentes(propostaId, dependentes)
        }
      }
    ...
    // Também salvar no campo JSON da proposta para compatibilidade
    try {
      await supabase
        .from("propostas")
        .update({
          dependentes_dados: JSON.stringify(dependentes),
          updated_at: new Date().toISOString(),
        })
        .eq("id", propostaId)
    } catch (updateError) {
      console.warn("⚠️ SALVAR DEPENDENTES - Erro ao atualizar campo JSON (não crítico):", updateError)
    }
 ...
 */
export async function salvarQuestionarioSaude(propostaId: string, respostas: any[]): Promise<any> {
  try {
    console.log(`🔍 SALVAR QUESTIONARIO - Salvando ${respostas.length} respostas para proposta ${propostaId}`)
    ...
    const respostasData = respostas.map((resp) => ({
      id: uuidv4(),
      proposta_id: propostaId,
    ...
    if (error) {
      ...
      if (error.message && error.message.includes("Invalid API key")) {
        ...
        if (reconectado) {
          // Tentar novamente após reconexão
          return await salvarQuestionarioSaude(propostaId, respostas)
        }
      }
...
/**
 * Salva os documentos de uma proposta
 */
export async function salvarDocumentos(propostaId: string, documentos: any): Promise<any> {
  try {
    console.log(`🔍 SALVAR DOCUMENTOS - Salvando documentos para proposta ${propostaId}`)
    console.log("🔍 SALVAR DOCUMENTOS - Documentos recebidos:", Object.keys(documentos))
    ...
    for (const [tipo, arquivo] of Object.entries(documentos)) {
      if (arquivo && arquivo instanceof File) {
        console.log(`🔍 SALVAR DOCUMENTOS - Processando documento ${tipo}:`, arquivo.name, `(${arquivo.size} bytes)`)

        try {
          // Gerar um nome de arquivo único
          const extensao = arquivo.name.split(".").pop()
          const nomeArquivo = `${propostaId}_${tipo}_${Date.now()}.${extensao}`

          // Fazer upload do arquivo para o Supabase Storage
          const { data, error } = await supabase.storage
            .from("documentos_propostas")
            .upload(`propostas/${nomeArquivo}`, arquivo, {
              cacheControl: "3600",
              upsert: true, // Permitir sobrescrever se necessário
            })

          if (error) {
            console.error(`❌ SALVAR DOCUMENTOS - Erro ao fazer upload do documento ${tipo}:`, error)
            errosUpload.push(`${tipo}: ${error.message}`)
            continue // Continuar com os próximos documentos mesmo se houver erro
          }

          // Obter a URL pública do arquivo
          const { data: urlData } = supabase.storage
            .from("documentos_propostas")
            .getPublicUrl(`propostas/${nomeArquivo}`)

          if (urlData && urlData.publicUrl) {
            documentosUrls[tipo] = urlData.publicUrl
            console.log(`✅ SALVAR DOCUMENTOS - Documento ${tipo} salvo com sucesso:`, urlData.publicUrl)
          } else {
            console.error(`❌ SALVAR DOCUMENTOS - Erro ao obter URL pública do documento ${tipo}`)
            errosUpload.push(`${tipo}: Erro ao obter URL pública`)
          }
        } catch (uploadError) {
          console.error(`❌ SALVAR DOCUMENTOS - Erro inesperado ao processar documento ${tipo}:`, uploadError)
          errosUpload.push(`${tipo}: ${uploadError.message}`)
        }
      } else if (arquivo) {
        console.warn(`⚠️ SALVAR DOCUMENTOS - Documento ${tipo} não é um arquivo válido:`, typeof arquivo)
        errosUpload.push(`${tipo}: Não é um arquivo válido`)
      } else {
        console.log(`ℹ️ SALVAR DOCUMENTOS - Documento ${tipo} não fornecido (opcional)`)
      }
    }

    // Verificar se pelo menos os documentos obrigatórios foram salvos
    const documentosObrigatorios = ["rg_frente", "rg_verso", "cpf", "comprovante_residencia"]
    const documentosObrigatoriosSalvos = documentosObrigatorios.filter((doc) => documentosUrls[doc])

    if (documentosObrigatoriosSalvos.length < documentosObrigatorios.length) {
      const faltando = documentosObrigatorios.filter((doc) => !documentosUrls[doc])
      console.error("❌ SALVAR DOCUMENTOS - Documentos obrigatórios não foram salvos:", faltando)
      throw new Error(`Falha ao salvar documentos obrigatórios: ${faltando.join(", ")}`)
    }

    if (errosUpload.length > 0) {
      console.warn("⚠️ SALVAR DOCUMENTOS - Alguns documentos tiveram problemas no upload:", errosUpload)
    }

    console.log("✅ SALVAR DOCUMENTOS - Documentos salvos com sucesso:", documentosUrls)
    return documentosUrls
  } catch (error) {
    console.error("❌ SALVAR DOCUMENTOS - Erro ao salvar documentos:", error)
    throw error
  }
}
